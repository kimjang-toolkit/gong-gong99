...
function App() {
    const router = createBrowserRouter([
        {
            path: '/',
            element: _jsx(Navigate, { to: "/co-buying", replace: true }),
        ...
        },
        {
            ...
            path: '/co-buying',
            children: [
                { index: true, element: _jsx(CoBuyingPage, {}) },
                { path: ':id', element: _jsx(DetailPage, {}) }, // 공구글 상세페이지
                { path: ':id/management', element: _jsx(ManagementPage, {}) }, // 공구글 관리페이지 , 추후 loader 추가 또는 private router씌우기
                { path: ':id?/password', element: _jsx(PasswordPage, {}) },
                { path: 'create', element: _jsx(CreatePage, {}) }, // 공구글 작성페이지
                // 비밀번호를 작성하는 페이지를 라우터로 관리해야할까? 상태관리는?
            ],
        },
    ]);
    return (_jsx(BaseLayout, { children: _jsx(RouterProvider, { router: router }) }));
}
export default App;
